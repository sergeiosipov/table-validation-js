...
| `comparison.fields.<col>.tolerance` — the four `ToleranceSpec` forms (absolute number, `{field, from}`, `{percent, of}`, `{fn}`) §15.8 | ComparisonPanel per-column **tolerance editor** (form selector + the chosen form's inputs); gated to `int`/`float`/`decimal` by rule C4 (`decimal` added 1.6.0; non-numeric columns show *numeric only*); the `exact` flag stays `int`/`float` (a decimal column is always exact — rules 59/C4, so no `exact` editor is offered); the `{fn}` form defers to **Advanced mode** (§9) for the registry (rule C5); inference `report.suggestions.tolerances` adopt buttons round-trip into it |
