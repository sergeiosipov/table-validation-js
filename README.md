...
[Browser JS profile](table-validation-js-impl-spec-v1.0.0.md) and the
[console UI architecture](table-validation-ui-architecture-v1.0.0.md) — live in this
repository.

[`dist/table-validation.js`](dist/table-validation.js) implements **Core Spec 1.0.0** in full
(`VERSION === SPEC_VERSION === "1.0.0"`): the validation engine, the `compare()` comparison engine,
...
1. Download the release archive of the pinned tag
   (`https://github.com/sergeiosipov/table-validation-js/archive/refs/tags/v1.0.0.zip`)
...
- [`console-standalone.html`](console-standalone.html) — the entire console in one file;
  the engine and console scripts load from `…@v1.0.0` on jsDelivr, each with a sha384
...
<script src="https://cdn.jsdelivr.net/gh/sergeiosipov/table-validation-js@v1.0.0/dist/table-validation.js"
        integrity="sha384-6Qg+Z7kzn+QhKYSzKRnoKakG5EI+gJLbXOyJV6JkAlGk251M30BTur4L3QJpI/KA" crossorigin="anonymous"></script> -->
```
...
|---|---|---|
| engine | `https://cdn.jsdelivr.net/gh/sergeiosipov/table-validation-js@v1.0.0/dist/table-validation.js` | `sha384-6Qg+Z7kzn+QhKYSzKRnoKakG5EI+gJLbXOyJV6JkAlGk251M30BTur4L3QJpI/KA` |
| worker wrapper | `https://cdn.jsdelivr.net/gh/sergeiosipov/table-validation-js@v1.0.0/dist/table-validation-worker.js` | `sha384-QjBLM9QZssz4XpPeJv7aSOp8uKTt1vBZYSfPLHjJbFGWltNsLCzgF49dqNKM3MXS` |
| TypeScript declarations | `https://cdn.jsdelivr.net/gh/sergeiosipov/table-validation-js@v1.0.0/dist/table-validation.d.ts` | — (not `<script>`-loaded) |
...
The dist artifacts are byte-identical to the original `v1.0.0` release, so the SRI
hashes are unchanged by the repository split.
...
`VERSION` (`"1.0.0"`), `SPEC_VERSION` (`"1.0.0"`).
...
implementing the [UI architecture spec](table-validation-ui-architecture-v1.0.0.md) on top of the
...
The specification set shares the unified version **1.0.0**, and
[`dist/table-validation.js`](dist/table-validation.js) implements it
(`VERSION === SPEC_VERSION === "1.0.0"`). The documents are split by audience:
...
- [Core Specification v1.0.0](https://github.com/sergeiosipov/table-validation-spec/blob/v1.0.0/table-validation-core-spec-v1.0.0.md) — behavior (normative), incl. the comparison engine (§15) and the §16 anchor for the addendum
- [Authoring, Ingestion & Inference Addendum v1.0.0](https://github.com/sergeiosipov/table-validation-spec/blob/v1.0.0/table-validation-authoring-tooling-addendum-v1.0.0.md) — normative core companion: config meta-model & builder (§A), `ingest()` + normalization pipeline (§B), `inferConfig()` (§C)
- [Design-Decisions Log](https://github.com/sergeiosipov/table-validation-spec/blob/v1.0.0/table-validation-design-decisions-v1.0.0.md) — non-normative record of every resolved ambiguity, with the genuine forks flagged
