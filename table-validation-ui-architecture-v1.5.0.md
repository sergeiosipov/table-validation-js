...
| `comparison.fields.<col>.tolerance` — the four `ToleranceSpec` forms (absolute number, `{field, from}`, `{percent, of}`, `{fn}`) §15.8 | ComparisonPanel per-column **tolerance editor** (form selector + the chosen form's inputs); gated to `int`/`float` by rule C4 (non-numeric columns show *numeric only*); the `{fn}` form defers to **Advanced mode** (§9) for the registry (rule C5); inference `report.suggestions.tolerances` adopt buttons round-trip into it |
