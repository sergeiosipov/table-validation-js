...
with sync_playwright() as p:
    ...
    assert "v1.3.1" in page.inner_text("#ver"), "engine version banner missing"
    ...
    for n in ("clean.config.json", "records.config.json"):
        ...
        assert cfg["meta"]["schemaVersion"] == "1.3.1" and cfg["columns"], f"{n} malformed"
    manifest = json.loads(z.read("manifest.json"))
    mstat = {f["file"]: f["status"] for f in manifest["files"]}
    assert mstat == st, f"manifest disagrees with the UI: {mstat}"
    assert manifest["files"] and manifest["engineVersion"] == "1.3.1"
