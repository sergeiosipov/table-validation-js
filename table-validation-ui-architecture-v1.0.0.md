...
- **Workers**: `validate()`/`compare()` and the exporters run in a Web Worker above ~10⁵ cells (the JS Profile §5 explicitly supports this — no DOM APIs, `Blob` exists in workers). `ingest()` likewise for byte sources. The store treats both paths identically (async actions); the UI thread only ever renders.
...
programmatic host bindings remain API-only. The benchmark harness (`test/bench.html`) and
the conformance suite (`test/index.html`) are separate pages by design, not console gaps.
...
| `inferConfig(table, options)` — `sampleRows`, `name`, `suggestRanges`, `seedComparison`, `allAcceptingFormats` §3.13 | InferenceBanner options row; evidence table; one-click adoption; Use-draft flow |
